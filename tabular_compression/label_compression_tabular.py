...
def makeTrainer(*,num_epochs=5,data_dir=None,projector=RoundedDoubleKronQR,k=24*8,
                bs=512,lr=3e-4,optim=Adam,device='cuda',trainer=Classifier,expt='',
    ...
    with FixedPytorchSeed(seed):
        ...
        model = MLP(cin, n_classes, hidden_dim=k, n_hidden=2).to(device)
...
if __name__=='__main__':
    ...
    with warnings.catch_warnings():
        ...
        cfg_spec.update({
            'd':[250,500,100,2000],
            'study_name':'tabular_label_compression','num_epochs':80,
