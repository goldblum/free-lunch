...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    model = build_model(args.arch, args.dataset_name, pretrained=args.pretrained, imagenet_resize = args.imagenet_resize)
    ...
    if not torch.cuda.is_available():
        ...
        if args.gpu is not None:
            ...
            model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model) #ADDED, DEBUG
        ...
        if (args.arch.startswith('alexnet') or args.arch.startswith('vgg')) and args.dataset_name=='ImageNet':
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch+1)%args.val_period == 0:
            acc1 = validate(val_loader, model, criterion, args, epoch=epoch)
        ...
        if (epoch+1)%args.save_period == 0 and (not args.multiprocessing_distributed or (args.multiprocessing_distributed
