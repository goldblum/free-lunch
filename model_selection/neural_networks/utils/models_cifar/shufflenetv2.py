...
class ShuffleUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, is_last=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.residual):
            ...
            if i == len(self.residual) - 1:
                preact = residual
        preact = torch.cat([shortcut, preact], dim=1)
...
class ShuffleNetV2(nn.Module):
    ...
    def forward(self, x, is_feat=False, preact=False):
        x = self.pre(x)
        f0 = x
        x, f1_pre = self.stage2(x)
        f1 = x
        x, f2_pre = self.stage3(x)
        f2 = x
        x, f3_pre = self.stage4(x)
        f3 = x
        x = self.conv5(x)
    ...
    def _make_stage(self, in_channels, out_channels, repeat):
        ...
        while repeat:
            layers.append(ShuffleUnit(out_channels, out_channels, 1, is_last=repeat==1))
            repeat -= 1
